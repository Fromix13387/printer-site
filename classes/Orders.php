...
class Orders
{
    ...
    {
        return $this->db->query('SELECT *,orders.created_at as created_at_or, orders.count as count_or,products.name as name_p FROM orders JOIN users ON orders.user_id = users.user_id JOIN products ON products.product_id = orders.product_id WHERE users.login = ? ORDER BY orders.created_at DESC ', [$login])->fetchAll();
    }
